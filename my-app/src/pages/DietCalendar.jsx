  ...
  return (
    <div className="dietPage">
      ...
      <svg className="dietBubbles" viewBox="0 0 100 100" preserveAspectRatio="none">
        ...
        {bubbles.map((b, i) => (
          <g key={i} className={`dietBubble dietSpeed${b.speed} dietDelay${b.delay}`} style={{ '--bx': `${b.x}px` }}>
            <circle className="dietBubbleOuter" cx={b.x} cy="96" r={b.r} stroke="url(#bubbleStrokeDiet)" />
            <circle className="dietBubbleHighlight" cx={b.x - 0.35} cy="95.6" r={Math.max(0.22, b.r * 0.28)} />
          </g>
        ))}
      </svg>
          ...
          <div className="dietCard pageCard section">
            <div className="dietGrid">
              {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d) => (
                <div key={d} className="dietDow">{d}</div>
              ))}
              ...
              {Array.from({ length: days }).map((_, i) => {
                ...
                return (
                  <button
                    key={key}
                    className={dayClass}
                    onClick={() => { if (!isFuture) navigate(`/diet/${key}`) }}
                    disabled={isFuture}
                  >
                    {dayNum}
        ...
        <div className="geminiModalOverlay" onClick={() => setAskOpen(false)}>
          <div className="geminiModal" onClick={(e) => e.stopPropagation()}>
              ...
              {messages.length === 0 ? (
                ...
                messages.map((m, i) => (
                  <div key={i} style={{ marginBottom: 10, textAlign: m.role === 'user' ? 'right' : 'left' }}>
                    <div style={{ fontWeight: 800, fontSize: 12, opacity: 0.8 }}>{m.role === 'user' ? 'You' : 'Gemini'}</div>
                    <div style={{
            ...
            <div className="geminiModalActions">
              <button className="btnGhost" onClick={() => setMessages([])} disabled={sending}>Clear chat</button>
              <button className="btnGhost" onClick={() => setAskOpen(false)}>Close</button>
              <button className="btnPrimary" onClick={handleSendGemini} disabled={sending}>{sending ? 'Sending…' : 'Send'}</button>
            </div>
          </div>
        </div>
      )}
